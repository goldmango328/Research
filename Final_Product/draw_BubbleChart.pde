...
void draw_BubbleChart(Table data, float x1,float y1,float x2,float y2){ 
  ...
  for(int i=0 ; i<data.getRowCount() ; i++){
    ...
    float data1 = row.getFloat("deaths_description");
    float data2 = row.getFloat("damage_description");
    
    float[] radius_MINMAX = getMinMax(data,"EQ_primary");
    float[] data1_MINMAX = getMinMax(data,"deaths_description");
    float[] data2_MINMAX = getMinMax(data,"damage_description");
    
    float x = map(data1, data1_MINMAX[0], data2_MINMAX[1], x1,x2);
    float y = map(data2, data2_MINMAX[0], data2_MINMAX[1], y1,y2);
